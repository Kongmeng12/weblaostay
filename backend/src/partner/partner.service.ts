...
export class PartnerService {
  ...
  async markRoomClean(partnerId: bigint, roomId: bigint) {
    ...
    const { count } = await this.prisma.rooms.updateMany({
      ...
      // `updated_at` is what tells the housekeeping sweep this room has been
      // seen to since its last guest left — the column carries no
      // `@updatedAt`, so Prisma will not bump it on its own.
